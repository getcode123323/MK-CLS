...
def update_precisions(Q,S,q,s,A,active,tol,n_samples,clf_bias):
    ...
    #fix any NA values in likelihood
    deltaL = np.nan_to_num(deltaL)
    ...
    if theta[feature_index] > 0:
        ...
        #print('will activate a feature index of '+str(feature_index))
...
class ClassificationARD(BaseEstimator,LinearClassifierMixin):
    ...
    def fit(self,X,y):
        ...
        if self.fit_intercept:
            ...
            #X = np.concatenate((np.ones([1,X.shape[1]]),X),0)
...
class RVC(ClassificationARD):
    ...
    def __init__(self, n_iter = 100, tol = 1e-4, n_iter_solver = 15, tol_solver = 1e-4,
                 ...
                 gamma  = None, coef0  = 1, kernel_params = None,given_K=False,kernel_test=None):
    ...
    def fit(self,X,y):
        ...
        if hasattr(self, 'given_K') == False:
            self.given_K = False


        if(self.given_K is False):
        ...
        if X.ndim == 1:
            #we don't want to use this relevant_vectors if we directly gives the K for training.
    ...
    def decision_function(self,X):
        ...
        if(self.given_K is False):
            kernel = lambda rvs : get_kernel(X,rvs,self.gamma, self.degree, 
                                             self.coef0, self.kernel, self.kernel_params)
            #such a over complicated implementation for such a simple problem.
        ...
        for rv,cf,act,b in zip(self.relevant_vectors_,self.coef_,self.active_,
            ...
            if rv.shape[0] == 0:
                ...
                if(self.given_K is False):
                    ...
                    decision.append(self._decision_function_active(kernel,cf,act,b))
        decision = np.asarray(decision).squeeze().T
        return decision
        

    def predict_proba(self,X,given_K=False):
        '''
