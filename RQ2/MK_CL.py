...
from imblearn.over_sampling import RandomOverSampler, BorderlineSMOTE, SVMSMOTE, SMOTE
...
def get_noise(xall_new, label_new, pre_new):
    ...
    for i in range(len(y_train2)):
        if y_train2[i]==0:
            ...
            num1=num1+1
    rate0=num0/(num1+num0)
    rate1=num1/(num1+num0)
    if rate1>rate0:
        o=0
    else:
        o=1
...
def trained_CL(X_train,Y_train,task,dataset,targetlabel,method):
   ...
   smote = SVMSMOTE(sampling_strategy='auto',svm_estimator=SVC(kernel='poly',degree=4,random_state=3),random_state=3)
